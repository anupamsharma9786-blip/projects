...
exports.DrugRiskController = void 0;
const DrugRiskService_1 = require("../services/DrugRiskService");
...
const drugRiskService = new DrugRiskService_1.DrugRiskService();
class DrugRiskController {
    /**
     * Process phenotypes and generate drug risks for a specific upload
     */
    async processDrugRisk(req, res) {
        try {
            ...
            if (!phenotypeRecord) {
                console.error(`[DrugRiskController] Phenotype record not found for uploadId: ${uploadId}`);
                res.status(404).json({
            ...
            console.log(`[DrugRiskController] Found phenotype record with ${phenotypeRecord.phenotypes.length} genes`);
            ...
            // Process drug risks
            const geneDrugRisks = drugRiskService.processDrugRisks(phenotypeRecord.phenotypes, selectedDrugs);
    ...
    async getDrugRisks(req, res) {
        try {
            ...
            const drugRiskRecord = await DrugRisk_1.DrugRiskModel.findOne({ uploadId });
            if (!drugRiskRecord) {
                res.status(404).json({
     ...
     */
    async getDrugRisksBySeverity(req, res) {
        try {
            const { uploadId } = req.params;
...
exports.DrugRiskController = DrugRiskController;
//# sourceMappingURL=DrugRiskController.js.map