...
import { Report, GeneReport } from '../models/Report';
...
export class ReportService {
   ...
   */
  async generateReport(uploadId: string): Promise<any> {
    ...
    // Get diplotypes
    const diplotypeData = await Diplotype.findOne({ uploadId });
    ...
    for (const geneDrugRisk of drugRisks.geneDrugRisks) {
      ...
      for (const drug of geneDrugRisk.drugs) {
        ...
        const dosageAdjustment = this.generateDosageSuggestion(
          geneDrugRisk.phenotype,
          drug.drug,
          drug.recommendation
        );
        ...
        const riskAnalysis = this.analyzePhenotypeRisk(
          geneDrugRisk.phenotype,
          drug.severity,
          drug.risk
        );
    ...
    let report = await Report.findOne({ uploadId });
    if (report) {
   ...
   */
  private getRiskLabel(severity: string, recommendation: string): string {
   ...
   * Generate dosage adjustment suggestions based on phenotype
   */
  private generateDosageSuggestion(phenotype: string, drug: string, recommendation: string): string {
   ...
   */
  private analyzePhenotypeRisk(phenotype: string, severity: string, risk: string): { analyzedSeverity: string; confidenceScore: number } {
    const phenotypeLower = phenotype.toLowerCase();
...
Format your response as JSON:
{
   ...
   */
  private async generateSummary(geneReports: GeneReport[]): Promise<string> {
...
Provide a patient-friendly summary that highlights key findings without being alarmist.`;
   ...
   * Get existing report
   */
  async getReport(uploadId: string): Promise<any | null> {
    const report = await Report.findOne({ uploadId });
    if (!report) {
      return null;
    }
