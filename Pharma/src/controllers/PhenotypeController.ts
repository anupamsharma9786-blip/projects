...
const phenotypeService = new PhenotypeService();

export class PhenotypeController {
  /**
   * Process diplotypes and generate phenotypes for a specific upload
   */
  async processPhenotype(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (!diplotypeRecord) {
        console.error(
          `[PhenotypeController] Diplotype record not found for uploadId: ${uploadId}`
        );
        res.status(404).json({
      ...
      console.log(
        `[PhenotypeController] Found diplotype record with ${diplotypeRecord.diplotypes.length} genes`
      );

      // Convert diplotypes to phenotypes
      const phenotypes = phenotypeService.convertDiplotypesToPhenotypes(
        diplotypeRecord.diplotypes
      );
   ...
   */
  async getPhenotypes(req: Request, res: Response): Promise<void> {
    try {
      const { uploadId } = req.params;
   ...
   */
  async getSupportedGenesWithPhenotypes(req: Request, res: Response): Promise<void> {
    try {
